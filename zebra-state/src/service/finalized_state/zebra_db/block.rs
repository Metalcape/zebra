...
impl ZebraDb {
    ...
    /// Returns the number of Sapling and Orchard transactions in the block with `hash_or_height`,
    /// if it exists in this chain.
    ///
    /// Returns `None` if the block is not found.
    pub fn shielded_tx_count(&self, hash_or_height: HashOrHeight) -> Option<(u64, u64)> {
        // Block
        let block = self.block(hash_or_height)?;
